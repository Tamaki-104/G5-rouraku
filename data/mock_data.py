"""
モックデータ（E-R図／設計書「５．データ設計（第三正規化）」に準拠）。

E-R図の各テーブルを、正規化された別々の dict リストとして保持する。
画面・ロジック側へは repository 層が結合（JOIN相当）して返すため、
Supabase へ移行しても同じ構造を再現できる。

  物件条件 (PropertyCondition) 1 ── N 住宅情報 (Properties)
  希望条件 (InputCondition)    1 ── N 入力情報 (Input)
