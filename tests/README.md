...
起動すると **2つのモード**を選べる（`manual_helpers.py` が入力を補助）。

- **a）自動モード**（Enter）… あらかじめ用意した全項目を一括実行し、各項目の
  「期待」と「戻り値」を表示する。回帰テスト・提出用。
- **m）手動モード** … テスターが引数（エリア・予算・間取り等）や経路（モック／実AI／
  取得失敗 等）を対話で入力し、通った分岐と戻り値を確認する。空Enterで既定値。
  例：matching で物件エリアを「横浜区」にすると県外分岐（適合度0%）を確認できる。
